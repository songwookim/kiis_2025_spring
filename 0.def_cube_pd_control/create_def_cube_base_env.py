...
import isaaclab.envs.mdp as mdps
import mdp as mdp
...
class CubeActionTerm(ActionTerm):
    ...
    def apply_actions(self):
        ...
        vel_error = -self._asset.data.root_vel_w
        # set velocity targets
        self._vel_command[:, :3] = self.p_gain * pos_error + self.d_gain * vel_error
        # self._asset.write_nodal_velocity_to_sim(self._vel_command)
        
         # 1. root_states에서 위치 정보만 추출 (처음 3개 차원)
        nodal_vels = self._vel_command[:, :3]  # Shape: [64, 3]

        # 2. 모든 노드에 동일한 위치를 할당하기 위해 차원 확장
        nodal_vels_expanded = nodal_vels.unsqueeze(1).expand(-1, 1331, -1)  # Shape: [64, 1331, 3]

        # 3. nodal_pos_w에 할당
        self._asset.data.nodal_vel_w[:] = nodal_vels_expanded
        # self._asset.data.nodal_vel_w[env_ids] = nodal_vel.clone()
        # set into simulation
        env_ids = slice(None)
        physx_env_ids = self._asset._ALL_INDICES
        self._asset.root_physx_view.set_sim_nodal_velocities(self._asset.data.nodal_vel_w[:], indices=physx_env_ids)
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    cube: DeformableObjectCfg = DeformableObjectCfg(
            prim_path="{ENV_REGEX_NS}/cube",
            init_state=DeformableObjectCfg.InitialStateCfg(pos=(0.0, 0.0, 5)),
...
class EventCfg:
    ...
    reset_base = EventTerm(
        func=mdps.reset_nodal_state_uniform,
        mode="reset",
        params={
            "position_range": {"x": (-0.5, 0.5), "y": (-0.5, 0.5)},
            "velocity_range": {
                "x": (-0.5, 0.5),
                "y": (-0.5, 0.5),
                "z": (-0.5, 0.5),
